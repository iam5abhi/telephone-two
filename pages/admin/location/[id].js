...
const Update = () => {
    ...
    const handleSubmit = (event) => {
      event.preventDefault();
        fetch("/api/location/update-location", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          body: JSON.stringify({ id: id ,location:location}),
        }).then(() => {
    ...
    const getCategotyData = () => {
        fetch("/api/location/get-singlelocation", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
          ...
          .then((data) => {
            ...
            setLocation(data.location); // Set the category state with the data
          })
          .catch((error) => {
                    ...
                    <div className="p-4">
                        <div className="w-full mx-auto md:w-[35%]">
                            <form className="bg-white shadow-xl py-14 rounded-lg p-5">
                                <h1 className=" font-bold text-3xl mb-8 text-center ">Update Location</h1>
                                <input type="hidden" name="remember" defaultValue="true" />
                                <div className="mt-8 space-y-4 ">
                                    <div className="mb-4">
                                        <label htmlFor="name" className="block mb-2 text-sm font-medium text-gray-900 ">Location</label>
                                        <input type="text" id="name" value={location} onChange={(e)=>setLocation(e.target.value)} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 " placeholder="Location" required />
                                    </div>
